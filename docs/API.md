...
**200 OK · application/json:** schemaVersion, release, channel, count, status, assetBase, bounds, maxHeightM, attribution, dataLicense, index, assets and sourceDatabase. `bounds` is [west, south, east, north] in WGS84 degrees. `index` contains fixed `zoom` (12), a tile path `template`, and `occupied` cells formatted as `x/y`. `assets` and `sourceDatabase` are resource paths.
