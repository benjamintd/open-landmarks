...
export function renderApiDocs(assets, catalogue) {
  const base = '/api/v1';
  const releaseBase = `${base}/releases/${catalogue.release}`;
  const pointer = { schemaVersion: 1, channel: catalogue.channel, release: catalogue.release, count: catalogue.count, catalogue: `${releaseBase}/catalogue.json` };
  ...
  const tileSample = { schemaVersion: 1, release: catalogue.release,
  ...
  const releaseParam = `${code('release')} — exact release ID returned by a pointer.`;
...
${[['release', 'Release'], ['catalogue', 'Catalogue'], ['tiles', 'Tiles'], ['metadata', 'Metadata'], ['models', 'Models'], ['sources', 'Sources']].map(([id, title]) => `<a href="#${id}">${title}</a>`).join('')}
</nav>
<p class="api-note">Resolve the global approved or preview channel, then request tiles from its catalogue. Fetch model files only when needed.</p>

${endpoint({ id: 'release', title: 'Get a release', path: '/api/v1/{channel}.json',
  description: 'Resolves a channel to its current release. Save the returned catalogue path to pin that release.',
  parameters: `${code('channel')} — ${code('latest')} for approved models, or ${code('preview')} to include drafts.`,
  response: `A release pointer. A channel with nothing published returns 200 with ${code('release: null')}, ${code('catalogue: null')} and ${code('count: 0')} — treat that as "not yet released" rather than an empty release. The current submissions have ${assets.filter(asset => asset.approved).length} approved models and ${assets.length} models in preview.`,
  cache: '60 seconds · revalidate', example: `${base}/preview.json`, sample: pointer,
  rows: [['schemaVersion', 'integer', 'Response schema version.'], ['channel', 'string', `${code('approved')} for latest.json; ${code('preview')} for preview.json.`], ['release', 'string | null', 'Immutable release ID, or null when nothing is published.'], ['count', 'integer', 'Number of models in this release.'], ['catalogue', 'string | null', 'Path to the pinned catalogue, or null when nothing is published.']] })}

${endpoint({ id: 'catalogue', title: 'Get a catalogue', path: '/api/v1/releases/{release}/catalogue.json',
  description: 'Returns spatial index information and links to the full dataset.', parameters: releaseParam,
  response: 'A catalogue for the requested release.', example: `${releaseBase}/catalogue.json`, sample: catalogue,
  rows: [['schemaVersion / release / channel / count', 'integer / string', 'Schema version, release ID, channel and model count, as in the release pointer.'], ['status', 'string', `${code('approved-only')} or ${code('includes-unreviewed-drafts')}.`], ['assetBase', 'string', 'Origin-relative asset root.'], ['bounds', 'number[4]', 'Dataset extent: [west, south, east, north], WGS84 degrees.'], ['maxHeightM', 'number', 'Maximum model height in metres; useful for padding discovery.'], ['index.zoom', 'integer', 'Fixed index zoom; currently 12.'], ['index.template', 'string', 'Tile path template with {x} and {y} placeholders.'], ['index.occupied', 'string[]', 'Available cells, each formatted as "x/y".'], ['assets', 'string', 'Path to all model metadata in this release.'], ['sourceDatabase', 'string', 'Path to the complete spatial source database.'], ['attribution / dataLicense', 'string', 'Dataset credit and spatial data license.']] })}

${endpoint({ id: 'tiles', title: 'Get a tile', path: '/api/v1/releases/{release}/index/12/{x}/{y}.json',
  ...
  rows: [['schemaVersion', 'integer', 'Response schema version.'], ['release', 'string', 'Pinned release ID.'], ['assets', 'object[]', 'Model entries; see metadata fields below.']] })}
...
${endpoint({ id: 'metadata', title: 'Get model metadata', path: '/assets/{id}/{publicationRevision}/asset.json',
  description: 'Returns placement, downloads, provenance and review information for one model. Follow the metadata path from a tile or the full dataset response.',
...
<section class="api-endpoint" id="sources"><h2>Dataset and sources</h2>
...
<p>There is one global dataset. No collection parameter is needed.</p><section class="api-endpoint" id="http"><h2>HTTP behavior</h2>
...
<p>The release pointers revalidate after 60 seconds. Release data, model files and source revisions cache for one year and retain their URLs. JSON can use negotiated HTTP compression; explicit .gz files require client decompression. V1 has no server-side search or query filters.</p></section>
